<template>
  <div class="min-h-screen bg-slate-900 text-slate-100 font-sans flex flex-col antialiased selection:bg-[#2563eb] selection:text-white">
    <!-- Top Global System Header Bar (Sophos SFOS Style) -->
    <header class="bg-slate-900 border-b border-slate-800 sticky top-0 z-50 flex-none shadow-sm">
      <!-- High-contrast corporate blue top accent bar -->
      <div class="h-1 w-full bg-[#2563eb]"></div>
      ...
      <div class="px-4 py-2.5 flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            ...
            class="lg:hidden inline-flex items-center justify-center p-2 rounded-md text-slate-400 hover:text-white hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-[#2563eb] transition-colors"
          ...
          <!-- Sophos XGS Appliance Brand Identity -->
          <div class="flex items-center gap-3 select-none">
            <div class="w-8 h-8 rounded bg-[#2563eb] flex items-center justify-center shadow-md font-extrabold text-white text-base tracking-wider ring-1 ring-blue-400/30">
            ...
            <div class="flex flex-col">
              <div class="flex items-center gap-2">
                <span class="text-sm font-black tracking-tight text-white uppercase">Sophos <span class="text-[#2563eb]">XGS</span></span>
                <span class="text-[10px] bg-blue-950 text-blue-400 font-mono font-bold px-1.5 py-0.5 rounded border border-blue-800/60">SFOS 20.0</span>
              </div>
        ...
        <div class="hidden md:flex flex-1 max-w-md mx-4">
          <div class="relative w-full">
            <input
              v-model="searchQuery"
              type="text"
              placeholder="Search features, policies, IPs, routing..."
              class="w-full bg-slate-950/80 text-slate-200 text-xs px-3 py-1.5 pl-8 rounded-md border border-slate-700/80 focus:outline-none focus:border-[#2563eb] focus:ring-1 focus:ring-[#2563eb] placeholder:text-slate-500 transition-colors"
            />
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <!-- Sophos XGS Dark Charcoal/Navy Sidebar Container (bg-slate-900) -->
      <aside
        :class="[
          'fixed lg:static inset-y-0 left-0 z-40 w-64 bg-slate-900 border-r border-slate-800 flex flex-col transition-transform duration-200 ease-in-out lg:translate-x-0 select-none shadow-xl lg:shadow-none',
          isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        ]"
        role="navigation"
        aria-label="Sophos SFOS Main Navigation"
      >
        <!-- Four Hardcoded Vertical Navigation Groupings -->
        <nav class="flex-1 overflow-y-auto py-3 px-2.5 space-y-4 scrollbar-thin scrollbar-thumb-slate-800">
          <div
          ...
          >
            ...
            <div class="px-2.5 pt-2 pb-1 text-[10px] font-bold tracking-wider text-slate-400/80 uppercase">
              {{ group.header }}
            </div>
                  ...
                  <div class="flex items-center gap-2.5 min-w-0">
                    <span
                      :class="[
                        ...
                        isItemActive(item.id) ? 'text-white' : 'text-slate-400 group-hover:text-blue-400'
                      ]"
                    >
        ...
        <div class="p-3 border-t border-slate-800 bg-slate-950/70 text-xs text-slate-400 flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="relative flex h-2 w-2">
              <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
              <span class="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
            </span>
            <span class="text-[11px] font-medium text-slate-300">SFOS Engine Active</span>
          </div>
          <span class="text-[10px] font-mono text-slate-500">XGS v20.0</span>
        </div>
      </aside>

      <!-- Light Gray Main Content Workspace Area (bg-slate-50) -->
      <main class="flex-1 overflow-y-auto bg-slate-50 text-slate-800 p-4 md:p-6 lg:p-8 flex flex-col space-y-6">
        ...
        <div class="bg-white border border-slate-200 rounded-lg p-4 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <nav class="flex items-center gap-2 text-xs font-mono text-slate-500 uppercase tracking-wider mb-1" aria-label="Breadcrumb">
              ...
              <span class="text-[#2563eb] font-bold">{{ currentItemTitle }}</span>
            </nav>

            <!-- Page Title with Corporate Blue Accent Mark -->
            <div class="flex items-center gap-2.5">
              <span class="w-1.5 h-6 bg-[#2563eb] rounded-sm inline-block"></span>
              <h1 class="text-xl font-black text-slate-900 tracking-tight">
                {{ currentItemTitle }}
              </h1>
              <span v-if="currentItemBadge" class="text-[11px] bg-blue-50 text-[#2563eb] font-medium font-mono px-2 py-0.5 rounded border border-blue-200">
                {{ currentItemBadge }}
              </span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2.5 flex-wrap">
            <slot name="header-actions">
              ...
              <button
                type="button"
                class="px-3.5 py-1.5 text-xs font-semibold bg-[#2563eb] hover:bg-blue-700 text-white rounded-md border border-blue-600 transition-colors shadow-sm flex items-center gap-1.5 active:bg-blue-800"
              >
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white border border-slate-200 rounded-lg p-6 shadow-sm">
              <div class="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
                <div>
                  <h2 class="text-base font-bold text-slate-900 flex items-center gap-2">
                    <span class="w-1.5 h-4 bg-[#2563eb] rounded-sm"></span>
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div class="p-4 bg-slate-50 border border-slate-200 rounded-md">
                  ...
                  <div class="text-lg font-mono font-bold text-[#2563eb] mt-1">{{ activeItemId }}</div>
                </div>
                <div class="p-4 bg-slate-50 border border-slate-200 rounded-md">
                  <div class="text-xs text-slate-500 font-medium uppercase">Protection Level</div>
                  <div class="text-lg font-bold text-emerald-600 mt-1">Enterprise XGS</div>
                </div>
              </div>

              <p class="text-sm text-slate-600 leading-relaxed">
                Sophos XGS navigation shell initialized. Use this container to host child modules such as Rules & Policies, Zenarmor Web Protection, Mail Manager, Nginx WAF, Network Interfaces, WireGuard VPN, and System Diagnostics.
              </p>
            </div>
          </slot>
        </div>

        <!-- Sophos SFOS Bottom Footer Bar -->
        <footer class="pt-4 border-t border-slate-200 text-xs text-slate-500 flex flex-col sm:flex-row items-center justify-between gap-2">
          <div class="flex items-center gap-2">
            <span class="font-bold text-slate-700">Sophos XGS Security Appliance</span>
            <span>&bull;</span>
            <span>SFOS Linux Kernel Core</span>
          </div>
...
const props = defineProps({
  ...
  applianceHost: {
    type: String,
    default: 'sophos-xgs.corp.internal'
  },
  cpuUsage: {
    type: Number,
    default: 18
...
// Sophos XGS (SFOS) Four Clean Vertical Navigation Groupings
const navGroups = [
  {
    id: 'monitor_analyze',
